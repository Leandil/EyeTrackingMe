            ...
            for i in range (0,len(ajall)):
                for j in range(0,len(ajall[ajfile[i]])-1):
                    if len(ajall[ajfile[i]][j]) == 5:    
                        index[ajfile[i]]= [ajall[ajfile[i]][j][1],ajall[ajfile[i]][j+1][1],int(ajall[ajfile[i]][j+1][2]),int(ajall[ajfile[i]][j+3][col]),int(ajall[ajfile[i]][-2][col])]
...
def get_etdata(n):
        ...
        global taboo,O3,O1,O2,tmt,tmt1,indo1,indo2,EX1,EY1,PUP1,EX2,EY2,PUP2,TMT,MM1,MM2, namenu, nmenu, menus_ind
...
def degtovel(sr):
        ...
        for i in range (0,len(ajfile)):
                ...
                MVEL1[ajfile[i]] = [mvela,mvelb, MDEG1[ajfile[i]][2],MDEG1[ajfile[i]][3]]
                MVEL2[ajfile[i]] = [mvelaa,mvelbb,MDEG2[ajfile[i]][2],MDEG2[ajfile[i]][3]]
...
def veltoacc(sr):
        ...
        for i in range (0,len(ajfile)):
                ...
                MACC1[ajfile[i]] = [macca,maccb, MDEG1[ajfile[i]][2],MDEG1[ajfile[i]][3]]
                MACC2[ajfile[i]] = [maccaa,maccbb,MDEG2[ajfile[i]][2],MDEG2[ajfile[i]][3]]
                macca,maccb, maccaa, maccbb = [],[],[],[]
...
def gnames():
        ...
        renomme name file 
        """
        global names, spl,spl1
        for i in range (0,len(ajfile)):
                spl = ajfile[i].split('_') ## split le nom originakl
                try:
                        int(spl[-1][0])
